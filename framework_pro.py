...
class NetworkScannerPlugin(BasePlugin):
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if ret == 0:
                    ...
                    return {"port": port, "service": service, "banner": banner or "(no banner)", "state": "open"}
                elif ret == 111 or ret == 61:  # Connection refused (Linux/macOS)
                    return {"port": port, "service": SERVICE_MAP.get(port, "unknown"), "banner": "", "state": "closed"}
                else:
                    return {"port": port, "service": SERVICE_MAP.get(port, "unknown"), "banner": "", "state": "filtered"}
    ...
    def execute(self, config: dict) -> dict:
        ...
        show_closed = Confirm.ask("[bold cyan]Afficher les ports fermés/filtrés ?[/bold cyan]", default=False)
        ...
        # ── Affichage style nmap ───────────────────────────────────────────
        tbl = Table(title=f"Résultats du scan — {target}", box=box.SIMPLE_HEAVY)
        tbl.add_column("Port",    style="bold cyan", width=8)
        tbl.add_column("État",    width=10)
        tbl.add_column("Service", style="yellow", width=14)
        tbl.add_column("Bannière / Info", style="white")

        display_results = all_results if show_closed else open_ports
        for r in display_results:
            ...
            tbl.add_row(str(r["port"]), state_color, r["service"], r["banner"] or "")

        if display_results:
            console.print(tbl)
            console.print(
                f"[dim]{len(open_ports)} port(s) ouvert(s), "
                f"{len([r for r in all_results if r['state'] == 'closed'])} fermé(s), "
                f"{len([r for r in all_results if r['state'] == 'filtered'])} filtré(s) "
                f"sur {len(ports)} scanné(s)[/dim]"
            )
        else:
            console.print("[bold red]Aucun port ouvert détecté.[/bold red]")

        return {"target": target, "ip": ip, "ports_scanned": len(ports), "open_ports": open_ports}
...
def _generate_pdf_report(report_data: dict, json_path: str) -> str | None:
    ...
    # Helper : encode latin-1 et purge les caractères hors-charset Helvetica
    ...
    pdf.set_xy(15, 30)
    pdf.cell(0, 12, "Enterprise Security Framework",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")

    pdf.set_font("Helvetica", "B", 18)
    pdf.set_text_color(200, 200, 200)
    pdf.cell(0, 10, "Rapport d'Audit de Securite",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")

    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(139, 148, 158)
    pdf.cell(0, 8, f"Version {report_data['metadata']['engine_version']}",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.cell(0, 8,
             f"Genere le {report_data['metadata']['scan_time'][:19].replace('T', ' a ')}",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    pdf.cell(0, 30, f"{score}/10",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")

    pdf.set_font("Helvetica", "B", 24)
    pdf.cell(0, 12, _safe(sev),
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    pdf.set_text_color(88, 166, 255)
    pdf.cell(0, 8, "Facteurs de risque :",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(200, 200, 200)
    for factor in risk.get("factors", [])[:10]:
        pdf.set_x(20)
        pdf.cell(0, 6, f"  > {_safe(factor)}",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.cell(0, 5,
             "Usage strictement autorise - sur systemes avec consentement ecrit explicite",
             align="C")
    ...
    for module_name, module_data in report_data.get("collected_evidence", {}).items():
        ...
        pdf.cell(0, 10, f"Module : {_safe(module_name)}",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.set_text_color(139, 148, 158)
        pdf.cell(0, 5,
                 f"Capture le {module_data.get('captured_at','')[:19].replace('T',' ')}",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if findings:
            ...
            for f in findings[:15]:
                ...
                rgb   = {
                    ...
                    "MOYEN":    (253, 216, 53),
                    "FAIBLE":   ( 67, 160, 71),
                }.get(sev_f, (150, 150, 150))
                pdf.set_text_color(*rgb)
                pdf.set_x(15)
                # ">" remplace "•" — Helvetica ne supporte pas l'Unicode étendu
                pdf.cell(0, 5, f"  > {_safe(f)[:100]}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.set_text_color(88, 166, 255)
        pdf.cell(0, 6, "Donnees collectees :",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        for key, val in payload.items():
            ...
            if isinstance(val, list):
                pdf.set_x(12)
                pdf.cell(0, 4.5, f"{_safe(key)}: [{len(val)} items]",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            elif isinstance(val, dict):
                pdf.set_x(12)
                pdf.cell(0, 4.5, f"{_safe(key)}: {{...}}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            else:
                pdf.set_x(12)
                pdf.cell(0, 4.5, f"{_safe(key)}: {_safe(val)}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            if pdf.get_y() > 270:
                break
